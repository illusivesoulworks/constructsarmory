        ...
        for (BlockPos.MutableBlockPos blockpos$mutableblockpos1 : BlockPos.getAllInBoxMutable(pos.add((double)(-f), -1.0D, (double)(-f)), pos.add((double)f, -1.0D, (double)f)))
        {
            if (blockpos$mutableblockpos1.distanceSqToCenter(living.posX, living.posY, living.posZ) <= (double)(f * f))
            {
                ...
                if (iblockstate.getMaterial() == Material.AIR)
                {
                    ...
                    if (iblockstate1.getMaterial() == Material.LAVA && iblockstate1.getBlock() == Blocks.LAVA && iblockstate1.getValue(BlockLiquid.LEVEL) == 0 && world.mayPlace(ConstructsArmor.softMagma, blockpos$mutableblockpos1, false, EnumFacing.DOWN, null))
                    {
                        world.setBlockState(blockpos$mutableblockpos1, ConstructsArmor.softMagma.getDefaultState());
                        world.scheduleUpdate(blockpos$mutableblockpos1.toImmutable(), ConstructsArmor.softMagma, MathHelper.getInt(living.getRNG(), 60, 120));
                    }
                }
            }
        }
